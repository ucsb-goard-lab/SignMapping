...
%             obj.data = data;
        ...
        function on_resp = separateSingleResponse(obj, stimdata, direction, rec)
            ...
            off_resp = squeeze(nanmean(off_resp, 3));
            for rep = 1:repeats
                on_resp(:, :, :, rep) = on_resp(:, :, :, rep) - off_resp(:, :, rep);
            end
        end
        ...
        function [aziResp, altResp] = separateResponseData(obj,raw_stimdata)        
            ...
            altResp = cat(4, altResp_u./obj.n_recordings, altResp_d./obj.n_recordings);
        end
...
%             dff = zeros(y_pixels, x_pixels, num_images,'single');
            ...
            for b = 1:num_blocks
                ...
                % now we have a block...
                frame_F(idx_vec) = mean(img_block, [1, 2]);
                block_median(:, :, b) = median(img_block, 3);
            end
            clear img_block % get rid of it for space concerns
