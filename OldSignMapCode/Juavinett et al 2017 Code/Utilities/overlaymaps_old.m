...
HorizRetContour=figure('Name','  Horizontal Retinotopy','NumberTitle','off','OuterPosition',[200, 200, 800, 800]);
    contour(kmap_hor1,[-70:20:70],'LineWidth',2)
...
imagesc(kmap_thresh);
title(strcat(ExptID,' Horizontal Retinotopy -LP ',num2str(LP(x))),'FontSize',14,'Interpreter','none')
colormap(threshmap)
...
%% save overlay
    ...
    AnalDir = strcat(Root_AnalDir,AzAnim,'\Overlays\');
    filename5 = strcat(ExptID,'_HorizRet1_LP',num2str(LP(x)));
    filename8 = strcat(ExptID,'_HorizRet2_LP',num2str(LP(x)));
    ...
    Root_AnalDir = 'C:\Analyzed Data_ISI\';
    AnalDir = strcat(Root_AnalDir,AzAnim,'\HorizRet\');
%         AnalDir = strcat(Root_AnalDir,AzAnim{g},'\');
    filename2 = strcat(ExptID,'_HorizRetRespMag_LP',num2str(LP(x)));
    filename6 = strcat(ExptID,'_HorizRetThresh_LP',num2str(LP(x)));
    filename1 = strcat(ExptID,'_HorizRetRaw');
    filename66 = strcat(ExptID,'_HorizRetContour');
    if iscell(AnalDir)
        AnalDir=AnalDir{1};
    end
    if exist(AnalDir) == 0
...
if x==2 %overlay only for 0.5 LP smoothing
...
title(strcat(ExptID,' Vertical Retinotopy -LP ',num2str(LP(x))),'FontSize',14,'Interpreter','none')
colormap(threshmap)
colorbar('EastOutside','FontSize',12,'YTick',[-30:10:70])
set(gcf,'Color','w');
set(gca,'FontName','arial');
...
%% save overlay
    Root_AnalDir = 'C:\Analyzed Data_ISI\';
    AnalDir = strcat(Root_AnalDir,AltAnim,'\Overlays\');
    filename11 = strcat(ExptID,'_VertRet1_LP',num2str(LP(x)));
    filename12 = strcat(ExptID,'_VertRet2_LP',num2str(LP(x)));
   filename10 = strcat(ExptID,'_VertAnatomy');
    if iscell(AnalDir)
        AnalDir=AnalDir{1};
    end
    if exist(AnalDir) == 0
        ...
        saveas(VertRet1,strcat(AnalDir,filename11,'.tif'))
        saveas(VertRet1,strcat(AnalDir,filename11,'.fig'))
        saveas(VertRet2,strcat(AnalDir,filename12,'.tif'))
        saveas(VertRet2,strcat(AnalDir,filename12,'.fig'))  
        saveas(Anatomy,strcat(AnalDir,filename10,'.tif'))
        saveas(Anatomy,strcat(AnalDir,filename10,'.fig'))
        
        %% save 

    %Paths for saving data and plots
    Root_AnalDir = 'C:\Analyzed Data_ISI\';
    AnalDir = strcat(Root_AnalDir,AltAnim,'\VertRet\');
%         AnalDir = strcat(Root_AnalDir,AltAnim{g},'\');
    filename1 = strcat(ExptID,'_VertRetRaw');
    filename2 = strcat(ExptID,'_VertRetRespMag_LP',num2str(LP(x)));
    filename6 = strcat(ExptID,'_VertRetThresh_LP',num2str(LP(x)));
    filename66 = strcat(ExptID,'_VertRetContour');
    if iscell(AnalDir)
        AnalDir=AnalDir{1};
    end
    if exist(AnalDir) == 0
