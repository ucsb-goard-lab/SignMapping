...
%get analyzer file
Dir = 'E:\AnalyzerFiles\';  %for new hard drive (march 2016)
Anadir=[Dir Anim,'\',ExptID '.analyzer']; 
load(Anadir,'Analyzer','-mat')

%get processed data
%pathname='C:\neurodata\Processed Data\';
pathname='E:\ProcessedData\';  %for new hard drive (march 2016)
...
if HP(x)==0
    ...
    axis tight
     ...
     filename18 = strcat(ExptID,'_HorizRet3_LP',num2str(LP(x)),'_HP ',num2str(HP(x)));       
...
VertRet2=figure('Name','  Vertical Retinotopy','NumberTitle','off');
   ...
   VertRet3=figure('Name','  Vertical Retinotopy','NumberTitle','off');
    ...
    filename22 = strcat(ExptID,'_VertRet3_LP',num2str(LP(x)),'_HP ',num2str(HP(x)));
    saveas(VertRet3,strcat(AnalDir,filename22,'.tif'))
    saveas(VertRet3,strcat(AnalDir,filename22,'.fig'))
   end 
