...
function [maps] =  C_widefieldSignMapNEW(aziFT_f,aziFT_b,altFT_u,altFT_d,ref_img,SaveDir)
...
scaling_factor = size(aziFT_f.mag,1)/size(ref_img_mat,1);
...
% frequency)
    ...
    if LP ~= 0
        L = fspecial('gaussian',15,LP);  %make LP spatial filter
...
%     delay_vert = delay_vert*180/pi.*bw;
    ...
    %%%
         horz_factor =  142/360; % this isn't worknig well right now, so eccentricity is not in degrees, but in arbitrary phase
         vert_factor = 128/360;
...
%             kmap_vert = downsample(rot90(kmap_vert),downsample_val);
    ...
    %saving sign map
    disp('Press any button to close maps and finish...')
    pause
